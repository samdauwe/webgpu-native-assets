...
layout (binding = 1) uniform textureCube textureEnvMap;
layout (binding = 2) uniform sampler samplerEnvMap;
...
layout (binding = 3) uniform Exposure {
	float exposure;
} exposure;
...
void main()
{
	...
	switch (constantId.type) {
		case 0: // Skybox
			{
				vec3 normal = normalize(inUVW);
				color = texture(samplerCube(textureEnvMap, samplerEnvMap), normal);
			}
			break;
		...
		case 1: // Reflect
			{
				...
				color = texture(samplerCube(textureEnvMap, samplerEnvMap), reflect(-wViewVec, wNormal));
		...
		case 2: // Refract
			{
				...
				color = texture(samplerCube(textureEnvMap, samplerEnvMap), refract(-wViewVec, wNormal, 1.0/1.6));
			}
			break;
	}
