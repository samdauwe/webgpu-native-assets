...
layout (binding = 1) uniform texture2D tColorMap;
layout (binding = 2) uniform sampler sColorMap;
layout (binding = 3) uniform texture2D tNormalHeightMap;
layout (binding = 4) uniform sampler sNormalHeightMap;
...
{
	float height = 1.0 - textureLod(sampler2D(tNormalHeightMap, sNormalHeightMap), uv, 0.0).a;
...
vec2 steepParallaxMapping(vec2 uv, vec3 viewDir) 
{
	...
	vec2 currUV = uv;
	float height = 1.0 - textureLod(sampler2D(tNormalHeightMap, sNormalHeightMap), currUV, 0.0).a;
	for (int i = 0; i < ubo.numLayers; i++) {
		currLayerDepth += layerDepth;
		currUV -= deltaUV;
		height = 1.0 - textureLod(sampler2D(tNormalHeightMap, sNormalHeightMap), currUV, 0.0).a;
...
vec2 parallaxOcclusionMapping(vec2 uv, vec3 viewDir) 
{
	...
	float prevDepth = 1.0 - textureLod(sampler2D(tNormalHeightMap, sNormalHeightMap), prevUV, 0.0).a - currLayerDepth + layerDepth;
...
void main(void) 
{
	...
	if (ubo.mappingMode == 0) {
		// Color only
		outColor = texture(sampler2D(tColorMap, sColorMap), inUV);
	} else {
		...
		vec3 normalHeightMapLod = textureLod(sampler2D(tNormalHeightMap, sNormalHeightMap), uv, 0.0).rgb;
		vec3 color = texture(sampler2D(tColorMap, sColorMap), uv).rgb;
