...
layout (binding = 1) uniform texture2D textureSmoke;
layout (binding = 2) uniform sampler samplerSmoke;
layout (binding = 3) uniform texture2D textureFire;
layout (binding = 4) uniform sampler samplerFire;
...
void main () 
{
	...
	{
		// Flame
		color = texture(sampler2D(textureFire, samplerFire), rotUV);
		outFragColor.a = 0.0;
	}
	else
	{
		// Smoke
		color = texture(sampler2D(textureSmoke, samplerSmoke), rotUV);
